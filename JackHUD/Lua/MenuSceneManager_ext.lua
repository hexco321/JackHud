if JackHUD and JackHUD._data then
	function MenuSceneManager:_get_lobby_character_prio_item(rank, outfit)
		...
		if JackHUD._data.lobby_skins_mode ~= 2 then
			...
			if (primary_rarity and secondary_rarity and primary_rarity >= secondary_rarity) or primary_rarity or (JackHUD._data.lobby_skins_mode == 3) then
				return "primary"
			elseif secondary_rarity or (JackHUD._data.lobby_skins_mode == 4) then
				return "secondary"
			end
		end
