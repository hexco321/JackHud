...
function WeaponLaser:update(unit, t, dt, ...)
	...
	if self._theme_type == "default" then
		if JackHUD:GetOption("enable_laser_options") then
			local r, g, b = JackHUD:GetOption("laser_color_r"), JackHUD:GetOption("laser_color_g"), JackHUD:GetOption("laser_color_b")
			if JackHUD:GetOption("laser_color_rainbow") then
				r, g, b = math.sin(135 * t + 0) / 2 + 0.5, math.sin(140 * t + 60) / 2 + 0.5, math.sin(145 * t + 120) / 2 + 0.5
			end
			self._themes.default = {
				light = Color(r, g, b) * JackHUD:GetOption("laser_light"),
				glow = Color(r, g, b) * JackHUD:GetOption("laser_glow"),
				brush = Color(JackHUD:GetOption("laser_color_a"), r, g, b)
			}
		else
